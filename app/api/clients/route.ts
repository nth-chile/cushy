...
export async function POST(req: Request) {
  ...
  const { name, company, hourlyRate, emailSubjectTemplate, emailBodyTemplate, contacts: contactList } = body;
  if (!name) return NextResponse.json({ error: "name required" }, { status: 400 });

  const [client] = await db
    .insert(clients)
    .values({ name, company, hourlyRate, emailSubjectTemplate, emailBodyTemplate })
    .returning();
